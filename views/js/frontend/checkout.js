...
jQuery(function ($) {
    ...
    var weareplanet_checkout = {
        ...
        height: null,
        element_processing_spinner_container: '#weareplanet-processing-spinner-container',
        processing: false,

        init : function () {
            $(this.element_processing_spinner_container).hide();
            this.handlerCounter = 0;
            var configuration = $('#weareplanet-method-configuration');
        ...
        register_method : function () {
            ...
            this.iframe_handler = window.weareplanetIFrameCheckoutHandler(this.configuration_id);
            ...
            this.iframe_handler.setInitializeCallback(function () {
                $('#weareplanet-iframe-possible').remove();
                $('.weareplanet-loader').remove();
                $('html, body').animate(
                    {
                        scrollTop : ($('#weareplanet-method-container').offset().top - screen.height/8)
                        },
                    1000
                );
                self.enable_pay_button();
            });
            this.iframe_handler.create("weareplanet-method-container");
        },

        register_button_handler : function () {
            $('.weareplanet-submit').off('click.weareplanet').on('click.weareplanet', {
                self : this
                }, this.handler_submit);
        },
        ...
        handle_iframe_height : function (height) {
            var item = $('#weareplanet-submit-top');
            if (height === 0) {
            ...
            if(!self.processing) {
                ...
                if (tosInput.size() > 0) {
                    if (!tosInput.is(':checked')) {
                        ...
                        self.show_new_errors(weareplanet_msg_tos_error);
                        self.enable_pay_button();
                        return false;
                    }
                }
        ...
        process_validation : function (validation_result) {
            ...
            if (validation_result.success) {
                ...
                var form = $('#weareplanet-payment-form');
                $.ajax({
                    type:       'POST',
                    dataType:   'json',
                    ...
                    success:    function (response, textStatus, jqXHR) {
                        ...
                        self.remove_existing_errors();
                        self.show_new_errors(weareplanet_msg_json_error);
                        self.hide_processing_spinner();
                        self.enable_pay_button();
                    },
        ...
        disable_pay_button : function () {
            $('.weareplanet-submit').prop('disabled', true);
            this.processing = true;
        },

        enable_pay_button : function () {
            $('.weareplanet-submit').prop('disabled', false);
            this.processing = false;
        },

        remove_existing_errors : function () {
            var error_message_selector = $('#weareplanet-error-messages');
